...
class Administrator(AbstractTable):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True, comment='id администратора')
...
class CourseToDog(AbstractTable):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True, comment='id связи собак с курсами')
    dog_id: Mapped[int] = mapped_column(ForeignKey("dogs.id", ondelete='CASCADE'), comment='id собаки')
    course_id: Mapped[int] = mapped_column(ForeignKey("courses.id", ondelete='CASCADE'), comment='id курса')
...
class Dog(AbstractTable):
    ...
    owner: Mapped[int] = mapped_column(ForeignKey("customers.id", ondelete='CASCADE', comment='id хозяина собаки'))
...
class Staff(AbstractTable):
    ...
    status: Mapped[int] = mapped_column(ForeignKey("staff_status.id", ondelete='SET NULL'), comment='id статуса сотрудника')
...
class StaffStatus(AbstractTable):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True, comment='id роли сотрудника')
